...
docker exec -i --user=1000:1000 $PROJECTNAME-apache sh -c 'º	drush dl -y --destination=/var/www/html --drupal-project-rename=drupal7 drupal && cd /var/www/html/drupal7 && drush si --db-url=mysql://root:123@db/drupal7 -y'