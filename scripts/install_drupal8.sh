...
docker exec -i --user=1000:1000 $PROJECTNAME-apache sh -c 'git clone https://git.drupal.org/project/drupal.git /var/www/html/drupal8 && cd /var/www/html/drupal8 && composer install && drush si --db-url=mysql://root:123@db/drupal8 --account-pass=admin -y'
